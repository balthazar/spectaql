...
module.exports = async options => {
  ...
  const loadData = async () => {
    const specPath = path.resolve(opts.specFile)
    delete require.cache[specPath]
    const specData = await require(opts.appDir + '/spectaql/makeSpec')(specPath)
    return require(path.resolve(opts.appDir + '/lib/preprocessor'))(options, specData)
  }

  const data = await loadData()
  const config = require(path.resolve(opts.configFile))(grunt, opts, data)
  ...
  var exists = grunt.file.exists(
    path.join(path.resolve('node_modules'), 'grunt-contrib-concat', 'package.json'),
  )
  ...
  grunt.registerTask('predentation', 'Remove indentation from generated <pre> tags.', function () {
    ...
    html = html.replace(/<pre.*?><code.*?>([\s\S]*?)<\/code><\/pre>/gim, function (x, y) {
      var lines = x.split('\n'),
        level = null
      if (lines) {
